...
def get_random_attributes(attr_list:dict, type:str) -> dict:
    """attr_list = attributes.attributes type = viking/english"""
    ...
    possible_attr_dict = attr_list[type].copy()
    attr_dict = {}
    for category in possible_attr_dict:
        attr_dict[category] = {}
        for i in range(num_attr[category]):

            random_attr = random.choice(list(possible_attr_dict[category].keys()))
